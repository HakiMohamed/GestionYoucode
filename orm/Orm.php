<?php
     ...
     class Orm {
        ...
        public function login($table, $password,$email)
        {
            try {
                
                $stmt = $this->db_database_connect->prepare("SELECT * FROM {$table} WHERE  mot_de_passe =:mot_de_passe  AND adresse_email= :adresse_email");
                $stmt->bindParam(":adresse_email", $email, PDO::PARAM_STR);
